...
def QC_2(opts, data):
    ...
    if not Path.is_file(Path(g1kDir + "1kG_MDS.bim")):
        # Name missing SNPs
        _, data = plink("--bfile {}1kG_MDS --set-missing-var-ids @:#[b37]\$1,\$2 --make-bed --out {}1kG_qc".format(g1kDir, outDir,), data)
        # Filter variants
	    # Remove variants based on missing genotype data
        _, data = plink("--bfile {}1kG_qc --geno 0.2 --allow-no-sex --make-bed --out {}1kG_qc".format(outDir, outDir), data)
        # Remove individuals based on missing genotype data
        _, data = plink("--bfile {}1kG_qc --mind 0.2 --allow-no-sex --make-bed --out {}1kG_qc".format(outDir, outDir), data)
        # Remove variants again
        _, data = plink("--bfile {}1kG_qc --geno 0.02 --allow-no-sex --make-bed --out {}1kG_qc".format(outDir, outDir), data)
        # Remove geno again
        _, data = plink("--bfile {}1kG_qc --mind 0.02 --allow-no-sex --make-bed --out {}1kG_qc".format(outDir, outDir), data)
        # Remove based on MAF
        _, data = plink("--bfile {}1kG_qc --maf 0.05 --allow-no-sex --make-bed --out {}1kG_qc".format(outDir, outDir), data)
        # Filter on HWE
        _, data = plink("--bfile {}1kG_qc --hwe 0.001 --allow-no-sex --make-bed --out {}1kG_qc".format(outDir, outDir), data)
    ...
    output.to_csv(sep="\t", path_or_buf='{}PopStrat_SNPs.txt'.format(outDir), index=False, header=False)

    # plink --bfile ${plinkFile}_7 --extract ${psDir}PopStrat_SNPs.txt --make-bed --out ${psDir}1kG
    output, data = plink("--bfile {}1kG_qc --extract {}PopStrat_SNPs.txt --make-bed --out {}1kG_qc".format(outDir, outDir, outDir), data)
    ...
    output = output[output.columns[1]]
    output.to_csv(sep="\t", path_or_buf='{}1kG_MDS_SNPs.txt'.format(outDir), index=False, header=False)
    ...
    output.to_csv(sep="\t", path_or_buf='{}buildReport.txt'.format(outDir), index=False, header=False)
    # plink --bfile ${psDir}1kG --update-map ${psDir}buildReport.txt --make-bed --out ${psDir}1kG_1

    output, data = plink("--bfile {}1kG_qc --update-map {}buildReport.txt --make-bed --out {}1kG_qc".format(outDir, outDir, outDir), data)
    ...
    output = output[cols]
    output.to_csv(sep="\t", path_or_buf='{}1kG1_tmp'.format(outDir), index=False, header=False)
    output = output[[output.columns[0], output.columns[1]]]
    output.to_csv(sep="\t", path_or_buf='{}1kG_ref-list.txt'.format(outDir), index=False,header=False)
    ...
    cols = [output.columns[i] for i in [1, 4, 5]]
    output = output[cols]
    output.to_csv(sep="\t", path_or_buf='{}PopStrat-adj_tmp'.format(outDir), index=False, header=False)
    ...
    output.to_csv(sep="\t", path_or_buf='{}all_differences.txt'.format(outDir), index=False, header=False)

    # # Flip SNPs for resolving strand issues
    # awk '{ print $1 }' ${psDir}all_differences.txt | sort -u > ${psDir}flip_list.txt
    output = output[[output.columns[0]]]
    output = output.drop_duplicates()
    output.to_csv(sep="\t", path_or_buf='{}flip_list.txt'.format(outDir), index=False, header=False)
    ...
    cols = [output.columns[i] for i in [1, 4, 5]]
    output = output[cols]
    output.to_csv(sep="\t", path_or_buf='{}PopStrat_corrected_tmp'.format(outDir), index=False, header=False)
    ...
    output.to_csv(sep="\t", path_or_buf='{}uncorresponding_SNPs.txt'.format(outDir), index=False, header=False)
    ...
    output = output[[output.columns[0]]]
    output = output.drop_duplicates()
    output.to_csv(sep="\t", path_or_buf='{}SNPs_excluded.txt'.format(outDir), index=False, header=False)
    ...
    output, data = plink("--bfile {}1kG_qc --exclude {}SNPs_excluded.txt --make-bed --out {}1kG_qc".format(outDir, outDir, outDir), data)
    ...
    output, data = plink("--bfile {}PopStrat_MDS2 --bmerge {}1kG_qc.bed {}1kG_qc.bim {}1kG_qc.fam --allow-no-sex --make-bed --out {}MDS_merge".format(outDir, outDir, outDir, outDir, outDir), data)
